...
func isSystemCall(caller types.Address) bool {
...
func (evm *EVM) DelegateCall(originCaller types.Address, caller types.Address, addr types.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	// Invoke tracer hooks that signal entering/exiting a call frame
	if evm.Config.Tracer != nil {
...
func (evm *EVM) create(caller types.Address, code []byte, gas uint64, value *uint256.Int, address types.Address, typ OpCode) (ret []byte, createAddress types.Address, leftOverGas uint64, err error) {
	if evm.Config.Tracer != nil {
	...
	if evm.StateDB.GetNonce(address) != 0 ||
		(contractHash != (types.Hash{}) && contractHash != types.EmptyCodeHash) || // non-empty code
		(storageRoot != (types.Hash{}) && storageRoot != types.EmptyRootHash) { // non-empty storage
		if evm.Config.Tracer != nil && evm.Config.Tracer.OnGasChange != nil {
			evm.Config.Tracer.OnGasChange(gas, 0, tracing.GasChangeCallFailedExecution)
		}
...
func (evm *EVM) initNewContract(contract *Contract, address types.Address) ([]byte, error) {
	...
	evm.StateDB.SetCode(address, ret, tracing.CodeChangeCreation)
	return ret, nil
}

// Create creates a new contract using code as deployment code.
func (evm *EVM) Create(caller types.Address, code []byte, gas uint64, value *uint256.Int) (ret []byte, contractAddr types.Address, leftOverGas uint64, err error) {
...
func (evm *EVM) Create2(caller types.Address, code []byte, gas uint64, endowment *uint256.Int, salt *uint256.Int) (ret []byte, contractAddr types.Address, leftOverGas uint64, err error) {
...
// Reset resets the EVM with a new transaction context and state database.
// This is used between transactions within a block.
func (evm *EVM) Reset(txCtx TxContext, statedb StateDB) {
	evm.TxContext = txCtx
	evm.StateDB = statedb
}

// AccountRef is a simple address wrapper to conform to the caller contract interface.
...
func (evm *EVM) captureBegin(depth int, typ OpCode, from types.Address, to types.Address, input []byte, startGas uint64, value *big.Int) {

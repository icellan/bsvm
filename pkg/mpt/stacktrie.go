...
type OnTrieNode func(path []byte, hash types.Hash, blob []byte)
...
func (t *StackTrie) Update(key, value []byte) error {
	...
	if err := t.insert(t.root, k, value, t.pBuf[:0]); err != nil {
		return err
	}
	return nil
}
...
func (t *StackTrie) insert(st *stNode, key, value []byte, path []byte) error {
	...
	case branchNode: /* Branch */
		...
		if st.children[idx] == nil {
			...
			if err := t.insert(st.children[idx], key[1:], value, append(path, key[0])); err != nil {
				return err
			}
		}
		return nil
	...
	default:
		return fmt.Errorf("invalid stack trie node type: %d", st.typ)
	}
}
...
func (t *StackTrie) hash(st *stNode, path []byte) {
	...
	default:
		// This should never happen in normal operation. If it does, the node
		// type is corrupted. We treat this as a no-op rather than panicking.
		return
	}
	...
	// to be retained.
	if t.onTrieNode != nil {
		t.onTrieNode(path, types.BytesToHash(st.val), blob)
	}
}
...
// right boundary.
func (t *StackTrie) Hash() types.Hash {
	n := t.root
	t.hash(n, nil)
	return types.BytesToHash(n.val)
}

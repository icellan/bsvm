...
type Node struct {
	Hash types.Hash // Node hash, empty for deleted node
	Blob []byte     // Encoded node blob, nil for the deleted node
}

// Size returns the total memory size used by this node.
func (n *Node) Size() int {
	return len(n.Blob) + types.HashLength
}
...
func New(hash types.Hash, blob []byte) *Node {
	return &Node{Hash: hash, Blob: blob}
}

// NewDeleted constructs a node which is deleted.
func NewDeleted() *Node { return New(types.Hash{}, nil) }
...
func (set *NodeSet) Merge(owner types.Hash, nodes map[string]*Node) error {
	if set.Owner != owner {
...
func (set *NodeSet) AddLeaf(parent types.Hash, blob []byte) {
...
func (set *NodeSet) HashSet() map[types.Hash][]byte {
	ret := make(map[types.Hash][]byte, len(set.Nodes))

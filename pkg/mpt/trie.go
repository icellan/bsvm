...
// Copy returns a copy of Trie. Returns an error if the trie contains
// corrupted nodes that cannot be copied.
func (t *Trie) Copy() (*Trie, error) {
	rootCopy, err := copyNode(t.root)
	if err != nil {
		return nil, err
	}
	return &Trie{
...
func New(id *ID, db NodeDatabase) (*Trie, error) {
	...
	if id.Root != (types.Hash{}) && id.Root != types.EmptyRootHash {
...
func (t *Trie) MustNodeIterator(start []byte) NodeIterator {
	...
	if err != nil {
		slog.Error("Unhandled trie error in Trie.NodeIterator", "err", err)
	}
	return it
}
...
func (t *Trie) MustGet(key []byte) []byte {
	res, err := t.Get(key)
	if err != nil {
		slog.Error("Unhandled trie error in Trie.Get", "err", err)
	}
	return res
}
...
func (t *Trie) get(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error) {
	...
	default:
		return nil, nil, false, fmt.Errorf("corrupted trie node: %T", origNode)
	}
}
...
func (t *Trie) MustGetNode(path []byte) ([]byte, int) {
	...
	if err != nil {
		slog.Error("Unhandled trie error in Trie.GetNode", "err", err)
	}
...
func (t *Trie) getNode(origNode node, path []byte, pos int) (item []byte, newnode node, resolved int, err error) {
	...
	if pos >= len(path) {
		...
		blob, err := t.reader.node(path, types.BytesToHash(hash))
	...
	default:
		return nil, nil, 0, fmt.Errorf("corrupted trie node: %T", origNode)
	}
}
...
func (t *Trie) MustUpdate(key, value []byte) {
	if err := t.Update(key, value); err != nil {
		slog.Error("Unhandled trie error in Trie.Update", "err", err)
	}
}
...
func (t *Trie) insert(n node, prefix, key []byte, value node) (bool, node, error) {
	...
	default:
		return false, nil, fmt.Errorf("corrupted trie node: %T", n)
	}
}
...
func (t *Trie) MustDelete(key []byte) {
	if err := t.Delete(key); err != nil {
		slog.Error("Unhandled trie error in Trie.Delete", "err", err)
	}
}
...
func copyNode(n node) (node, error) {
	...
	case nil:
		return nil, nil
	case valueNode:
		return valueNode(types.CopyBytes(n)), nil

	case *shortNode:
		valCopy, err := copyNode(n.Val)
		if err != nil {
			return nil, err
		}
		return &shortNode{
			flags: n.flags.copy(),
			Key:   types.CopyBytes(n.Key),
			Val:   valCopy,
		}, nil
	case *fullNode:
...
func (t *Trie) Hash() types.Hash {
	return types.BytesToHash(t.hashRoot().(hashNode))
}
...
func (t *Trie) Commit(collectLeaf bool) (types.Hash, *trienode.NodeSet, error) {
	defer func() {
		t.committed = true
	}()
	...
	committed, err := newCommitter(nodes, t.tracer, collectLeaf).Commit(t.root, t.uncommitted > 100)
	if err != nil {

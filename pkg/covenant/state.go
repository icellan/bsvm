...
// 32 bytes (StateRoot) + 8 bytes (BlockNumber) + 1 byte (Frozen) = 41 bytes.
const covenantStateEncodedSize = 41

// Encode serializes the covenant state for embedding in the locking script.
// The encoding is a fixed-size 41-byte blob:
//
...
// It expects exactly 41 bytes in the format produced by Encode.
